...
const SLIDES = [
    ...
    },
    {
        ...
        description: "Monitor and manage your content pipeline with real-time queue management and quality control.",
...
export default function LoginPage() {
    ...
    return (
        <div className="min-h-screen bg-black flex overflow-hidden font-sans select-none">
            <div className="w-full lg:w-[45%] flex flex-col p-8 md:p-20 justify-between items-center relative z-10 text-center border-r border-white/5">
                <div className="w-full max-w-sm flex flex-col items-center flex-1 justify-center">
                    <div className="w-12 h-12 bg-primary rounded-[2px] flex items-center justify-center mb-12 shadow-[0_0_30px_rgba(204,255,0,0.3)]">
                    ...
                    {!isEmailView ? (
                        ...
                        <form onSubmit={handleLogin} className="w-full space-y-4 animate-in fade-in zoom-in-95 duration-300">
                            <input
                                ...
                                className="w-full h-14 bg-[#111113] border border-white/5 rounded-[2px] px-5 text-white placeholder:text-slate-800 focus:outline-none focus:border-primary transition-all font-mono text-sm uppercase italic"
                                value={email}
                            ...
                            <input
                                ...
                                className="w-full h-14 bg-[#111113] border border-white/5 rounded-[2px] px-5 text-white placeholder:text-slate-800 focus:outline-none focus:border-primary transition-all text-sm uppercase italic"
                                value={password}
                                onChange={(e) => setPassword(e.target.value)}
                            />
                            <button
                                type="submit"
                                className="w-full bg-primary text-black font-[900] h-14 rounded-[2px] mt-4 shadow-[0_10px_30px_rgba(204,255,0,0.15)] hover:scale-[1.01] active:scale-95 transition-all uppercase tracking-widest text-xs italic"
                            >
                                Initiate Session
                            </button>
                            <button
            ...
            <div className="hidden lg:flex flex-1 h-screen relative bg-surface">
                ...
                <div className="absolute bottom-0 left-0 right-0 z-20 px-20 pb-16 flex items-end justify-between">
                    ...
                    <div className="flex flex-col items-end gap-1">
                        ...
                        <span className="text-[11px] font-black text-primary italic uppercase tracking-[0.3em]">NATAK.CORE_v3.5</span>
                    </div>
                </div>
            </div>
...
const SocialButton = ({ icon, text, onClick }: { icon: React.ReactNode, text: string, onClick?: () => void }) => (
    <button
        type="button"
        onClick={onClick}
        className="w-full h-14 bg-transparent border border-white/10 rounded-[2px] flex items-center justify-center gap-3 text-white font-black uppercase tracking-widest text-[10px] hover:bg-white/5 transition-all active:scale-[0.98]"
    >
