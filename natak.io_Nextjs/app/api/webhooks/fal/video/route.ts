/**
 * Fal.ai Video Webhook Handler
 * Receives callback when async video generation completes
 */
...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        // Fal.ai webhook can have different payload structures
        ...
        if (status === "FAILED" || error) {
            await supabase.from("jobs").update({
                status: "failed",
                error: typeof error === 'string' ? error : JSON.stringify(error) || "Video generation failed",
        ...
        if (status === "COMPLETED" || output) {
            ...
            try {
                ...
                console.error("R2 backup failed, using original URL:", r2Error);
                // Continue with original URL if R2 fails
            }
        ...
        // Status update (IN_PROGRESS, IN_QUEUE, etc)
