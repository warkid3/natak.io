...
const StatusCard = ({ label, status, icon }: { label: string, status: 'operational' | 'degraded' | 'outage', icon: React.ReactNode }) => (
    <div className="bg-[#1A1A1D] border border-white/5 p-8 rounded-sm flex items-center justify-between">
...
const FAQItem = ({ question, answer }: { question: string, answer: string }) => {
    ...
    return (
        <div className="bg-[#1A1A1D] border border-white/5 rounded-sm overflow-hidden">
            <button
            ...
            >
                ...
                <ChevronDown className={cn("w-4 h-4 text-slate-700 transition-transform duration-300", open && "rotate-180 text-primary")} />
            </button>
            {open && (
    ...
    return (
        <div className="h-full flex flex-col space-y-10 overflow-y-auto custom-scrollbar pr-2 bg-black font-sans w-full p-2">
            <header className="flex-shrink-0 bg-surface p-12 rounded-sm border border-white/5 flex justify-between items-end shadow-2xl">
                ...
                <div className="flex items-center gap-4 bg-black/50 px-8 py-4 border border-primary/20 rounded-sm">
                    <div className="w-2.5 h-2.5 rounded-full bg-primary animate-pulse"></div>
                    <span className="text-[11px] font-[900] text-primary uppercase italic tracking-[0.2em]">Live Operator Assistance: Active</span>
                </div>
            </header>

            <section className="grid grid-cols-1 lg:grid-cols-3 gap-10 px-12">
                ...
                <section className="space-y-6">
                    <div className="flex justify-between items-center mb-8">
                        ...
                        <Button
                            ...
                            variant="natak"
                            className="px-8 py-3 h-auto text-[10px] shadow-xl shadow-primary/10"
                        >
                            Open New Ticket
                        </Button>
                    </div>
                    ...
                    <div className="space-y-3">
                        {tickets.map(tk => (
                            <div key={tk.id} className="bg-[#1A1A1D] border border-white/5 rounded-sm p-6 flex justify-between items-center group hover:border-white/20 transition-all">
                                <div className="flex flex-col">
                                ...
                                <div className={cn(
                                    "px-4 py-1.5 rounded-sm text-[9px] font-black uppercase tracking-widest italic",
                                    tk.status === 'open' ? 'bg-primary/10 text-primary border border-primary/20' : 'bg-black text-slate-700 border border-white/5'
                                )}>
                                    {tk.status}
                                </div>
                            </div>
                        ...
                        {tickets.length === 0 && (
                            <div className="h-40 border border-dashed border-white/5 rounded-sm flex items-center justify-center text-slate-900 italic font-black uppercase tracking-[0.5em]">
                                Logs Clear
                            </div>
                        )}
                    </div>
            ...
            {/* Ticket Modal */}
            {showForm && (
                <div className="fixed inset-0 bg-black/95 backdrop-blur-xl z-[500] flex items-center justify-center p-6 animate-in fade-in duration-300 transition-all">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-8">
                            <div className="space-y-3">
                                ...
                                <select
                                    ...
                                    className="w-full bg-[#1A1A1D] border border-white/5 rounded-sm px-6 py-4 text-xs font-mono text-white italic focus:outline-none"
                                >
                            ...
                            <div className="space-y-3">
                                ...
                                <Input
                                    ...
                                    className=" bg-[#1A1A1D] border border-white/5 rounded-sm px-6 py-5 h-auto text-sm font-medium text-white focus-visible:ring-offset-0 focus-visible:ring-primary focus:border-primary transition-all uppercase italic"
                                    value={subject}
                            ...
                            <div className="space-y-3">
                                ...
                                <Textarea
                                    ...
                                    className="bg-[#1A1A1D] border border-white/5 rounded-sm px-6 py-5 text-sm font-medium text-white focus-visible:ring-offset-0 focus-visible:ring-primary focus:border-primary transition-all italic resize-none"
                                    value={description}
                            ...
                            <Button
                                type="submit"
                                variant="natak"
                                className="w-full py-6  flex items-center justify-center gap-4 shadow-xl shadow-primary/10 h-auto"
                            >
